...
describe('Polygon ZK-EVM', () => {
    ...
    beforeEach('Deploy contract', async () => {
        ...
        const PolygonZkEVMFactory = await ethers.getContractFactory('PolygonZkEVMMockDA');
    ...
    it('should sequence a batch as trusted sequencer', async () => {
        const batchHash = '0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470';
        ...
        await expect(polygonZkEVMContract.sequenceBatches([sequence], [batchDAData], trustedSequencer.address))
            .to.be.revertedWith('OnlyTrustedSequencer');

        // revert because tokens were not approved
        await expect(polygonZkEVMContract.connect(trustedSequencer).sequenceBatches([sequence], [batchDAData], trustedSequencer.address))
        ...
        // Sequence batch
        await expect(polygonZkEVMContract.connect(trustedSequencer).sequenceBatches([sequence], [batchDAData], deployer.address))
    ...
    it('sequenceBatches should sequence multiple batches', async () => {
        ...
        // Sequence batches
        await expect(polygonZkEVMContract.connect(trustedSequencer).sequenceBatches([sequence, sequence2], [batchDAData, batchDAData2], trustedSequencer.address))
            .to.emit(polygonZkEVMContract, 'SequenceBatches')
